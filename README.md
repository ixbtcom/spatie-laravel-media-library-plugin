...
3. Плагин проверяет, совпадают ли временный и целевой диски:
   - Записывается задание в очередь (`ProcessAsyncMediaFileMoveJob`), которое в фоне переместит файл из временной папки в целевую директорию `mediaId` и установит публичный доступ.
...
- **Умное определение режима обработки**: (Deprecated) — теперь все перемещения выполняются асинхронно через очередь, что гарантирует стабильность работы даже с большими файлами
- **Мгновенное перемещение**: При работе в пределах одного диска операции выполняются быстрее благодаря отсутствию лишних сетевых вызовов
...
- (Обновлено) Перемещение всегда осуществляется асинхронно — это обеспечивает более стабильную работу и не блокирует HTTP-запрос
