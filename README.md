...
2. Компонент создает запись Media в базе данных и связывает ее с моделью.
3. Запускается фоновая задача `ProcessAsyncMediaFileMoveJob`, которая:
   - Перемещает файл из временного хранилища в целевое хранилище
   - Обновляет запись Media с окончательными параметрами
   - Генерирует конверсии и адаптивные изображения, если они настроены
