...
3. Создается задание в очередь (`ProcessAsyncMediaFileMoveJob`), которое выполняется в фоновом режиме и:
   - Перемещает файл из временной папки в целевую директорию (определяемую ID записи медиа)
   - Устанавливает публичный доступ к файлу
   - Обновляет запись Media в базе данных (удаляет временные свойства)
   - Вызывает событие `MediaHasBeenAddedEvent` для инициирования дальнейшей обработки
4. HTTP-запрос возвращается пользователю немедленно, не дожидаясь завершения асинхронной операции
...
- Оптимизированная передача данных между разными дисками и S3/Ceph бакетами
- Корректная обработка ошибок и повторные попытки для надежности
- Использование `MediaHasBeenAddedEvent` для корректной интеграции с системой обработки медиа
...
- **Асинхронная обработка**: Все операции с файлами выполняются в фоновом режиме через очереди, что не блокирует основной процесс и UI
- **S3/Ceph Direct Copy**: Если исходный и целевой диски являются S3 или Ceph, используется API для прямого копирования файлов на стороне сервера
...
- **Надежные транзакции**: Задача запускается только после фиксации транзакции (afterCommit), что гарантирует наличие записи Media в базе

#### Интеграция с Bunny CDN

В последнем обновлении добавлены следующие улучшения:

- **Событийная архитектура**: После успешного перемещения файла вызывается событие `MediaHasBeenAddedEvent`, которое позволяет запускать дополнительную обработку (например, загрузку видео в Bunny CDN)
- **Использование правильных методов для custom properties**: Все операции с custom properties теперь используют рекомендованные методы `setCustomProperty()`, `getCustomProperty()` и `hasCustomProperty()` вместо прямого доступа к JSON-полю
- **Улучшенная обработка событий**: Стандартизирован подход к обработке событий жизненного цикла медиа-файлов с разделением ответственности между компонентами системы

#### Особенности работы с S3 и Ceph

Плагин включает специальную обработку для работы с облачными хранилищами:

- **Умная обработка публичного доступа**: Распознает S3/Ceph диски и учитывает особенности публичного доступа
- **Совместимость с Ceph**: Автоматическое определение S3-совместимых хранилищ с кастомными endpoint'ами
- **Проверка блокировки публичного доступа**: Определяет настройки доступа на уровне бакета (для AWS S3)
- **Валидация результата**: После установки публичного доступа проверяется фактическая видимость файла
- **Детальное логирование**: Каждый шаг работы с хранилищем логируется для упрощения отладки

> **Важно:** Для корректной работы публичного доступа в S3/Ceph, убедитесь, что:
...
Если в вашей конфигурации заблокирован публичный доступ на уровне бакета, вы можете:
...
- **Интеграция с S3 и Ceph**: Полная поддержка хранения файлов в облачных хранилищах
...
- **Асинхронная загрузка**: Обработка файлов в фоновом режиме через очереди Laravel без блокировки UI
- **Оптимизация для S3/Ceph**: Прямое копирование между бакетами для больших файлов
...
- **Транзакционная безопасность**: Гарантия запуска задач только после фиксации транзакций через afterCommit
